"""
    ALPModule:
    Reference: "Self-supervision with superpixels: Training few-shot medical image segmentation without annotation, ECCV 2020"
...
class MultiProtoAsConv(nn.Module):
    ...
    def forward(self, qry, sup_x, sup_y, mode, thresh=0.95, isval = False, val_wsize = None, vis_sim = False, **kwargs):
        """
        Now supports
        Args:
        ...
        # sup_x = sup_x.unsqueeze(0) # [nshot, nc, h, w]
        # sup_y = sup_y.unsqueeze(0) # [nshot, 1, h, w]
        ...
        # no need to merge with gridconv+
            ...
            sy_max = sup_y_g.max()
            sy_mean = sup_y_g.mean()
            protos = n_sup_x[sup_y_g > (sy_max*0.8), :] # npro, nc
            protos_b = n_sup_x[sup_y_g < sy_mean, :] # npro, nc
            pro_n = safe_norm(protos)
            pro_n_b = safe_norm(protos_b)
            pros = []
            b_pros = []
            for i in range(protos.shape[-2]):
                pros.append(pro_n[i])
            for i in range(protos_b.shape[-2]):
                b_pros.append(pro_n_b[i])

            qry_n = safe_norm(qry)
            pred_grid = None
            if pro_n.shape[0] != 0:
                dists = F.conv2d(qry_n, pro_n[..., None, None])
                pred_grid = torch.sum(F.softmax(dists, dim=1) * dists, dim=1, keepdim=True)

            return pros, b_pros, pred_grid
            ...
            dists = F.conv2d(qry_n, pro_n[..., None, None]) * 20 # 0710
